"use client";

import { motion, AnimatePresence } from "framer-motion";
import { z } from "zod";
import { cn } from "@/lib/utils";

// =============================================================================
// ZOD VALIDATION SCHEMAS
// =============================================================================

const FloatingActionButtonPropsSchema = z.object({
  whatsappNumber: z.string().optional(),
  whatsappMessage: z.string().optional(),
  buttonLabel: z.string().optional(),
  className: z.string().optional(),
});

// =============================================================================
// TYPES
// =============================================================================

export type FloatingActionButtonProps = z.infer<typeof FloatingActionButtonPropsSchema>;

// =============================================================================
// COMPONENT
// =============================================================================

export default function FloatingActionButton({
  whatsappNumber,
  whatsappMessage,
  buttonLabel = "Contact us on WhatsApp",
  className,
}: FloatingActionButtonProps) {
  // Validate props during development
  if (process.env.NODE_ENV === "development") {
    const result = FloatingActionButtonPropsSchema.safeParse({
      whatsappNumber,
      whatsappMessage,
      buttonLabel,
      className,
    });
    if (!result.success) {
      console.warn("[FloatingActionButton] Prop validation warning:", result.error.flatten());
    }
  }

  // Determine URL
  const whatsappUrl = whatsappNumber
    ? `https://wa.me/${whatsappNumber}${
        whatsappMessage ? `?text=${encodeURIComponent(whatsappMessage)}` : ""
      }`
    : null;

  if (!whatsappUrl) return null;

  return (
    <AnimatePresence>
      <div
        className={cn(
          "fixed bottom-6 right-6 md:bottom-8 md:right-8 z-9999 isolate animate-in zoom-in fade-in duration-300",
          className
        )}
      >
        <a
          href={whatsappUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="group block relative outline-none focus:outline-none"
          aria-label={buttonLabel}
        >
          <span className="absolute right-full mr-3 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-deep-brown text-white text-sm font-medium rounded-lg opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 group-focus-visible:opacity-100 group-focus-visible:translate-x-0 transition-all duration-200 whitespace-nowrap shadow-md pointer-events-none">
            {buttonLabel}
            {/* Arrow */}
            <span className="absolute top-1/2 right-[-4px] -translate-y-1/2 border-y-4 border-y-transparent border-l-4 border-l-deep-brown" />
          </span>

          <motion.button
            style={{ position: "relative" }}
            className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-[#25D366] hover:bg-[#128C7E] shadow-2xl flex items-center justify-center text-white transition-colors focus-visible:ring-4 focus-visible:ring-[#25D366]/40 outline-none"
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            initial={{ rotate: 0 }}
            animate={{
              rotate: [0, -10, 10, -10, 10, 0],
              transition: {
                delay: 2,
                duration: 2.5,
                repeat: Infinity,
                repeatDelay: 5,
              },
            }}
          >
            <span className="sr-only">{buttonLabel}</span>
            <svg
              className="w-6 h-6 md:w-8 md:h-8 fill-current"
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
            </svg>
          </motion.button>
        </a>
      </div>
    </AnimatePresence>
  );
}
